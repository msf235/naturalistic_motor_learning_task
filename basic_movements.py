...
def reflective_random_walk(
    ...
    for i in range(1, n_steps):
        ...
        if new_position < lower_lim:
            ...
            new_position = (
                2 * upper_lim - new_position
            )  # Reflect off the upper boundary
        positions[i] = new_position

    # Smooth the positions using Gaussian filter
    smoothed_positions = gaussian_filter1d(
        positions, sigma=smoothing_sigma, mode="nearest"
    )
...
def throw_traj(model, data, Tk):
    ...
    arc_traj_vs = arc_traj(
        data.site(RSHOULD_S).xpos, r, np.pi, np.pi / 2.5, Tk - Tk2 - 1, density_fn=""
    )
