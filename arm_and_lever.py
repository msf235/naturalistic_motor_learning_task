...
def save_video(frames, name, framerate=60):
...
def forward_sim(
    model, data, taus, site1="tip", site2=None, renderer=None, frames=None, framerate=60
):
    if renderer is None:
    ...
    data.ctrl[:] = taus[0]
    epsilon = 1e-6
    mj.mjd_transitionFD(model, data, epsilon, True, As[0], Bs[0], None, None)
    ...
    for tk in range(1, Tk):  # Step the simulation.
        ...
        data.ctrl[:] = taus[tk]
        mj.mjd_transitionFD(model, data, epsilon, True, As[tk], Bs[tk], None, None)
...
def contact_one_it(
    ...
    for k, tk in enumerate(range(Tk - 2, Tk - m - 1, -1)):
        ...
        grads[tk] = (tau_loss_factor / (k + 1) ** 0.5) * loss_taus[tk] + Bs[
            tk
        ].T @ lams[tk + 1]
...
def contact_target(
    ...
    for k in range(K + 1):
        ...
        curr_loss = 0.5 * np.linalg.norm(dlds) ** 2
        lever_loss = 0.5 * (qs[Tk - 1, 2] - theta_final) ** 2
...
def optimize_traj_all(
    ...
    for k in range(K + 1):
        try:
            ...
            lever_loss = 0.5 * (qs[tk_targ1, 2] - theta_final) ** 2
            lever_loss += 0.5 * (qs[tk_targ3, 2] - theta_final) ** 2
            ...
            for k2, tk in enumerate(range(Tk - 2, Tk - m - 1, -1)):
                ...
                tau_grad = (tau_loss_factor / (k2 + 1) ** 0.5) * loss_taus[tk]
...
if __name__ == "__main__":
    ...
    taus_all = double_tap(
        num_its=(1000, 1000, 1000, 2000),
        lrs=(1, 1, 1, 1),
        tau_loss_factor=3e-6,
        angle_factor=1,
    )
