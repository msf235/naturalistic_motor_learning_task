...
def save_video(frames, name, framerate=60):
    ...
    out = cv2.VideoWriter(f'{name}.mp4', cv2.VideoWriter_fourcc(*'mp4v'),
                          framerate, (size[1], size[0]), True)
    for frame in frames:
        out.write(frame)
    out.release()
    out = cv2.VideoWriter(f'most_recent.mp4', cv2.VideoWriter_fourcc(*'mp4v'),
                          framerate, (size[1], size[0]), True)
    for frame in frames:
        out.write(frame)
    out.release()
