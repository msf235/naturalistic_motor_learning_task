...
class SchedulerInitializer(object):
    ...
    def __call__(self):
        ...
        param = self.param
        if param is None:
            scheduler = sched.ConstantScheduler(self.lr)
        elif isinstance(param, sched.SchedulerBase):
            scheduler = param
    ...
    def init_from_str(self):
        ...
        if "constant" in sch_str:
            scheduler = sched.ConstantScheduler(**kwargs)
        elif "exponential" in sch_str:
            scheduler = sched.ExponentialScheduler(**kwargs)
        elif "noam" in sch_str:
            scheduler = sched.NoamScheduler(**kwargs)
        elif "king" in sch_str:
            scheduler = sched.KingScheduler(**kwargs)
        else:
    ...
    def init_from_dict(self):
        ...
        if sc and sc["id"] == "ConstantScheduler":
            scheduler = sched.ConstantScheduler()
        elif sc and sc["id"] == "ExponentialScheduler":
            scheduler = sched.ExponentialScheduler()
        elif sc and sc["id"] == "NoamScheduler":
            scheduler = sched.NoamScheduler()
        elif sc:
